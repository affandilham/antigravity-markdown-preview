...
export function codeBlockPlugin(md: MarkdownIt, options?: { lineNumbers?: boolean }): void {
  ...
  md.renderer.rules.fence = (tokens, idx, _options, _env, _self) => {
    ...
    if (lang === 'mermaid' || lang === 'plantuml' || lang === 'puml') {
      // Let dedicated plugins handle them or render container
    ...
    if (showLineNumbers) {
      ...
      // If ends with newline, don't count extra empty line
    ...
    const displayLang = (lang || validLang || 'code').toUpperCase();
    const encodedRaw = encodeURIComponent(code);

    return `<div class="antigravity-code-block" data-lang="${lang || 'text'}">
